...
extern "C" {
    ...
    WASM_IMPORT("arduino", "getInitialFreeHeapSize") uint32_t getInitialFreeHeapSize (void);
...
const char* filename = "/tasks/performance.wasm";
...
void _setup() {
    int initialFreeHeapSize = getInitialFreeHeapSize();
    print("Initial free heap size: "); println(initialFreeHeapSize);
    int finalFreeHeapSize;
    println("Start!");
    int res = 0;
    for (int i = 0; i < 1501; i++) {
        ...
        if (i == 1499) {
            ...
            finalFreeHeapSize = getFreeHeapSize();
            print("Final free heap size: "); println(finalFreeHeapSize);
        }
    }
    ...
    print("RAM used: ");
    printInt(initialFreeHeapSize - finalFreeHeapSize);
    println(" bytes");
}
...
WASM_EXPORT
void _pause() {
    println("Calling pause operation - Performance Test");
}