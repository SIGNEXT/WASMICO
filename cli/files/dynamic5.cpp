...
const char* filename = "/tasks/dynamic5.wasm";
uint8_t state[4];
...
void _setup() {
    print("Free heap size after starting task 5: "); println(getFreeHeapSize());
    println("Dynamic task 5 started!");
    iterationCounter = 0;
...
void _loop() {
    ...
    iterationCounter++;
    print("Dynamic5 ran iteration "); println(iterationCounter);
    delay(5000);
    if (fib_res < 0) {
        println("Something");
    }
...
WASM_EXPORT
void _pause() {
    println("Calling pause operation - Dynamic Test 5");
