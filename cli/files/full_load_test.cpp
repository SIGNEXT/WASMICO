...
extern "C" {
    ...
    WASM_IMPORT("arduino", "getThreadNo")     int getThreadNo     (void);
...
const char* filename = "/tasks/full_load_test.wasm";
...
void _setup() {
    ...
    int delayMs = 38000 - ((threadNo - 1) * 2000);
    print("Delay: "); println(delayMs);
    delay(delayMs);
    println(threadNo);
    int res = 0;
    for (int i = 0; i < 1500; i++) {
...
WASM_EXPORT
void _pause() {
    println("Calling pause operation - Load Test");
}